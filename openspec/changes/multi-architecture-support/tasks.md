...
- [ ] 1.1 Create `saeforge/adapters/__init__.py` with `register_adapter`,
      `adapter_for`, `registered_classes`, and an internal `_REGISTRY`
      list of `(host_class, adapter)` tuples (first-match-wins).
- [ ] 1.2 Create `saeforge/adapters/base.py` with the
...
- [ ] 1.3 Wire `saeforge/adapters/__init__.py` to import the three
...
- [ ] 3.2 Move `_build_torch_module`'s GPT-2 logic from `saeforge/model.py`
...
- [ ] 3.3 Move `_config_from_host`'s GPT-2 logic into
      `GPT2Adapter.build_native_config`. The result has `family="gpt2"`.
- [ ] 3.4 Replace `SubspaceProjector.project_module`'s body with the
      adapter dispatch (`adapter_for(host).walk(host, self,
      attention_width=...)`); keep the lazy `transformers` import +
      friendlier `ImportError`.
- [ ] 3.5 Replace `NativeModel.from_host`'s `GPT2LMHeadModel.from_pretrained`
      with `transformers.AutoModelForCausalLM.from_pretrained`; route
      the projection through the adapter dispatch.
- [ ] 3.6 Replace `_build_torch_module(config)` body with a dispatch on
      `config.family`; the GPT-2 branch calls `_build_gpt2_module`.
- [ ] 3.7 In `saeforge/forge.py:185`, replace `GPT2LMHeadModel.from_pretrained`
      with `AutoModelForCausalLM.from_pretrained` so non-GPT-2 hosts
      load as their actual class.
- [ ] 3.8 Run the full suite — every existing GPT-2 test must pass
      unchanged.

## 4. Llama adapter

- [ ] 4.1 Create `saeforge/adapters/llama.py` with `LlamaAdapter`.
- [ ] 4.2 Implement `walk(host, projector, attention_width)`:
...
- [ ] 4.3 Implement `build_native_config(host, n_features,
...
- [ ] 4.4 Implement `native_module_class()` returning a `ForgedLlama`
      class built by a `_build_llama_family_module(config)` helper.
- [ ] 4.5 Implement `_build_llama_family_module(config)` (in
...
- [ ] 4.6 Register `LlamaAdapter` for `transformers.LlamaForCausalLM`.

## 5. Gemma-2 adapter

- [ ] 5.1 Create `saeforge/adapters/gemma2.py` with `Gemma2Adapter`
      that subclasses `LlamaAdapter` (or imports its helpers) and
      overrides `walk`, `build_native_config`, and
      `native_module_class`.
- [ ] 5.2 Extend `walk` to emit the two extra per-layer RMSNorm keys
      (`pre_feedforward_layernorm.weight`,
      `post_feedforward_layernorm.weight`).
- [ ] 5.3 Extend `build_native_config` to copy
      `final_logit_softcapping` and `attn_logit_softcapping` from
      `host.config` into the matching `NativeModelConfig` fields. Set
      `family="gemma2"`.
- [ ] 5.4 Extend `_build_llama_family_module` (or branch in a Gemma-2
...
- [ ] 5.5 Register `Gemma2Adapter` for `transformers.Gemma2ForCausalLM`.

## 6. Unregistered architecture handling

- [ ] 6.1 `saeforge.adapters.adapter_for` raises `NotImplementedError`
      naming the host's type and the registered class names when no
      match is found.
- [ ] 6.2 `SubspaceProjector.project_module` propagates this error
      verbatim (no longer raises its own GPT-2-specific
      `NotImplementedError`).

## 7. Tests

- [ ] 7.1 Add `tiny_llama` fixture to `tests/conftest.py` (`LlamaConfig`
...
- [ ] 7.2 Add `tiny_gemma2` fixture (`Gemma2Config` with
      `final_logit_softcapping=30.0, attn_logit_softcapping=50.0`).
- [ ] 7.3 Add `tests/test_subspace_projector.py::test_llama_walker_keys_and_shapes`
      and the GQA-shape audit (`q_proj` rows = `n_q_heads * head_dim`;
      `k_proj` / `v_proj` rows = `n_kv_heads * head_dim`).
- [ ] 7.4 Add `test_llama_tied_embeddings_omits_lm_head`.
- [ ] 7.5 Add `tests/test_subspace_projector.py::test_gemma2_walker_emits_four_norms_per_block`
      and the soft-cap-config-passthrough audit.
- [ ] 7.6 Add `tests/test_subspace_projector.py::test_unregistered_architecture_raises`
...
- [ ] 7.7 Add `tests/test_native_model.py::test_llama_native_module_state_dict_matches_walk`
...
- [ ] 7.8 Add `test_native_model_config_requires_family` and
      `test_native_model_config_rejects_unknown_family`.

## 8. Examples

- [ ] 8.1 Verify `examples/forge_gemma2_2b.py` runs end-to-end with
...
- [ ] 8.2 Add `examples/forge_synthetic_llama.py` — runs against a
...
- [ ] 8.3 Add `tests/test_examples_smoke.py` invoking the synthetic
...
- [ ] 9.1 Switch `transformers.GPT2LMHeadModel.from_pretrained` to
      ...
      stays explicit).
- [ ] 9.2 Add a unit test (without HF download) that constructs a
...
- [ ] 10.1 Update `README.md` "Hardware notes" tier table:
      v0.2 supports GPT-2 family + Llama-3 + Gemma-2; Pythia /
      GPT-NeoX explicitly deferred.
- [ ] 10.2 Update `README.md` "Integration with Polygram" section to
      reflect that Llama / Gemma support landed.
- [ ] 10.3 Update `docs/algorithm.md` v0 implementation notes to
      mention RMSNorm / SwiGLU / GQA equivalent projection paths and
      the soft-cap / sliding-window non-goals for Gemma-2.
- [ ] 10.4 CHANGELOG entry under "Added (multi-architecture-support)"
      naming the new adapters, the `family` field, and the breaking
      `NativeModelConfig` migration.

## 11. Verification

- [ ] 11.1 Run `pytest -q` and confirm all tests pass (existing GPT-2
      green, new Llama / Gemma-2 green).
- [ ] 11.2 Run `ruff check saeforge tests examples` clean.
- [ ] 11.3 Run `openspec validate multi-architecture-support --strict`
      clean.
- [ ] 11.4 Spot-check the adapter dispatch by constructing a real
