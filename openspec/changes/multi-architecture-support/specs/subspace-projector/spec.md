...
The semantics of these keys and shapes are unchanged from v0.1; the
implementation has moved from a hard-coded walker inside
`SubspaceProjector` into `saeforge.adapters.gpt2.GPT2Adapter.walk`.
`project_module` now dispatches via `saeforge.adapters.adapter_for(host)`
and returns the adapter's walk output verbatim.
...
whose class has no registered adapter in `saeforge.adapters`. The error
message SHALL name the offending type and SHALL list the registered
class names so the user can see which architectures are supported. The
v0.1 message format (containing `"GPT-2"`) is replaced — the new format
names the unregistered type and the full registered set.

#### Scenario: an unregistered host class is rejected with an actionable message

- **GIVEN** a custom class `FakeBert` (not registered in
  `saeforge.adapters`)
