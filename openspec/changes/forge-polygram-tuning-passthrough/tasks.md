...
- [ ] 1.1 In `pyproject.toml`, bump the `polygram` minimum version to the
      release shipping `polygram-tuning-config`. Add a comment naming the
      change so future readers can grep.
- [ ] 1.2 In `saeforge/actions/__init__.py`, wrap `from polygram import
...
- [ ] 3.1 In `_build_context` (`saeforge/forge.py`), serialise each
...
- [ ] 3.2 In `compress_with_polygram`
      ...
      `config=`.
- [ ] 3.3 In `perform_regrowth` (`saeforge/actions/__init__.py:103`):
...
- [ ] 4.1 Update `saeforge/machines/sae_forge.orca.md` context schema:
...
- [ ] 4.2 Run `orca verify saeforge/machines/sae_forge.orca.md` and
      confirm zero exit.

## 5. CLI

- [ ] 5.1 Add `--coverage-target`, `--cosine-threshold`,
...
- [ ] 5.2 Add `--regrow-layer`, `--regrow-strategy` flags. When either
      is supplied — or when `--regrow-count > 0` — build a
      `RegrowConfig` and pass to `ForgePipeline(regrow=...)`.
- [ ] 5.3 In the CLI's argparse setup, mark `--regrow-layer` as
...
- [ ] 6.1 Delete `compression_strategy: str = "merge"` and
      `rep_selection: str = "scale_aware"` from `ForgePipeline`
      (`saeforge/forge.py:55-56`).
- [ ] 6.2 Delete the matching ctx-build lines (`saeforge/forge.py:247-248`).
- [ ] 6.3 Audit any in-tree caller passing those fields:
      `examples/`, `tests/`, `scratch/`. Migrate each to
      `compression=CompressionConfig(...)`.

## 7. Examples

- [ ] 7.1 In `examples/forge_gpt2_real_sae.py:125-138`, replace the
...
- [ ] 7.2 Verify the example runs end-to-end and produces a checkpoint
      whose first-batch eval roughly matches the pre-change baseline
      (within whatever tolerance the example asserts).

## 8. Tests

- [ ] 8.1 Add `tests/test_polygram_tuning_passthrough.py` covering:
...
- [ ] 9.3 Add a YAML example to `docs/` showing
      `ForgePipeline.from_dict(yaml.safe_load(...))` end-to-end.

## 10. Verification

- [ ] 10.1 Run `pytest -q` and confirm all tests pass.
- [ ] 10.2 Run `examples/forge_gpt2_real_sae.py` end-to-end on the
      bundled toy SAE and confirm it converges.
- [ ] 10.3 Run `openspec validate forge-polygram-tuning-passthrough
      --strict` and confirm clean exit.
- [ ] 10.4 Coordinate with polygram's `polygram-tuning-config` merge:
