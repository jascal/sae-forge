...
- [ ] 1.1 New module `saeforge/circuit_heads.py`. `prev_token_heads(host, corpus, *, top_k=4, ctx=96) -> list[tuple[int,int]]`: one forward pass with `attn_implementation="eager"`, return the `top_k` heads by mean Δ=1 (previous-token) attention
...
- [ ] 3.3 `_augmented_report` records the writer heads used + the mode (`writer-output` vs `reader-geometry`)
