# polygram-tuning-passthrough Specification

## Purpose

The `polygram-tuning-passthrough` capability defines how `ForgePipeline`
exposes polygram's typed tuning configurations
(`CompressionConfig`, `EpochCompressionConfig`, `RegrowConfig`) as
first-class fields, round-trips them through the orca-runtime FSM
context as JSON-friendly dicts, and surfaces a small set of CLI flags
plus a `from_dict(...)` loader for YAML/JSON configs. It bridges
